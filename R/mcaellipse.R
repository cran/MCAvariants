mcaellipse <-
function (Xtable, a1=1,a2=2,alpha = 0.05, cols = c(2, 4),np=5,nmod=c(5,5,5,5,5), M = 2, cex =0.8, cex.lab = 0.8, mar = c(5, 4, 4, 2) + 0.1, 
prop=0.8,Imass,Jmass,a,b,g,fr,dmu,inertiapc,
pos=2, graphy=TRUE,ell=FALSE) 
{
    I <- nrow(Xtable)
    J <- ncol(Xtable)
n<-sum(Xtable)
...
colgrlab <- list(1,"","+","red","T")
    Inames <- dimnames(Xtable)[[1]]
    Jnames <- dimnames(Xtable)[[2]]
    dIh <- solve(Imass^0.5)
  #  dJh <- solve(Jmass^0.5)
  dJh <-Jmass # radice quadrata inversa in uscita di mcabasic    
t.inertia <- sum(dmu)
dmu<-sqrt(dmu)
dftot=1/2*(J-np)^2-sum((nmod-1)^2)
#chisq.val <- qchisq(1 - alpha, df = (I - 1) * (J - 1))
chisq.val <- qchisq(1 - alpha, df = dftot)
...
abline(h = 0, v = 0) 
#text(fr[, a1], fr[, a2], labels = Inames, pos= pos, col = cols[1], cex = cex)
#points(fr[, a1], fr[, a2], pch="*",col=cols[1],cex=cex)
    text(g[, a1], g[, a2], labels = Jnames, pos= pos, col = cols[2], 
        cex = cex)
...
#browser() 
 (list(eccentricity = eccentricity, row.summ = row.summ, 
        col.summ = col.summ,dftot=dftot,pvalcol=pvalcol))
}
